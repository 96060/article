...
<script>
	function deleteArticle(id) {
		$('.popup-article,.popup-bg').css('display', 'block');
		$('.popup form input[name="id"]').val(id);
	}

	function deleteArticleReply(id) {
		$('.popup-reply,.popup-bg').css('display', 'block');
		$('.popup form input[name="id"]').val(id);
	}
...
<body>
	...
	<div>
		...
		<a href="./list.sbs">글 리스트</a> <a href="./add.jsp">글 작성하기</a> <a
			href="./modify.sbs?id=<%=id%>">글 수정하기</a> <a
			href="javascript:deleteArticle(<%=id%>);">글 삭제하기</a>
	...
	<form action="./doAddReply.sbs" method="POST"
		...
		<div>
			...
			<input type="submit" value="댓글작성"> <input type="reset"
				value="취소">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					...
					<div class="read-mode-visible"><%=articleReply.get("body")%></div>
				...
				<td><a
					href="javascript:deleteArticleReply(<%=articleReply.get("id")%>);">삭제</a>
